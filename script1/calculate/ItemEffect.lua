...
--Ticket to Sacred Snow Mountain票
...
--Ticket to Lone Tower Entrance票
...
function ItemUse_Zhenheilong( role , Item )	----------真黑龙宝箱
	...
	GiveItem ( role , 0 , 2367  , 1 , 18 ) 
	GiveItem ( role , 0 , 2368  , 1 , 18 ) 
	GiveItem ( role , 0 , 2369  , 1 , 18 )
        local cha_type = GetChaTypeID ( role )
	if cha_type == 4 then 
		GiveItem ( role , 0 , 2370  , 1 , 18 )		
	end
end
--Ticket Abbadon 4票
...
function ItemUse_ZSCard(role, Item )
	local i = CheckBagItem(role,2941) -- rebirth card
	local k = ChaIsBoat(role)
	local Item_CanGet = GetChaFreeBagGridNum ( role )
	local charLv=Lv ( role )
	if Item_CanGet < 1 then
	...
	elseif k == 0 then
		if i > 0 then
			...
			if j == 1 then
				GiveItem ( role , 0 , 2235 , 1 , 42) -- rebirth stone
...
function ItemUse_pirategem( role , Item )
	local Item_CanGet = GetChaFreeBagGridNum ( role )	
	if Item_CanGet <= 2 then
		...
		--SystemNotice(role ,"湾漕耱囹铟眍 戾耱� � 桧忮眚囵�. 湾忸珈铈眍 铗牮� 洋礓箨 项觐痂蝈�� 祛疱� 漓� 70 箴钼��")
		--SystemNotice(role ,"Insufficient slot in inventory. Failed to use Lv 70 Ami Voyager Chest")
		UseItemFailed ( role )
		return
	end
	GiveItem ( role , 0 , 5886 , 1 , 102) 
	GiveItem ( role , 0 , 5887 , 1 , 102)  
	GiveItem ( role , 0 , 5888 , 1 , 102)  
	GiveItem ( role , 0 , 5889 , 1 , 102) 
	GiveItem ( role , 0 , 5890 , 1 , 102)  
	GiveItem ( role , 0 , 5891 , 1 , 102) 
	GiveItem ( role , 0 , 5892 , 1 , 102)
	GiveItem ( role , 0 , 5893 , 1 , 102)  
	GiveItem ( role , 0 , 5894 , 1 , 102) 
	GiveItem ( role , 0 , 5895 , 1 , 102)  
	GiveItem ( role , 0 , 5896 , 1 , 102) 
	GiveItem ( role , 0 , 5897 , 1 , 102)
end

function ItemUse_pirategem2( role , Item )
	local Item_CanGet = GetChaFreeBagGridNum ( role )	
	if Item_CanGet <= 2 then
		SystemNotice(role ,"\205\229\228\238\241\242\224\242\238\247\237\238 \236\229\241\242\224 \226 \232\237\226\229\237\242\224\240\229. \205\229\226\238\231\236\238\230\237\238 \238\242\234\240\251\242\252 \209\243\237\228\243\234 \207\238\234\238\240\232\242\229\235\255 \236\238\240\229\233 \192\236\232 70 \243\240\238\226\237\255")
		--SystemNotice(role ,"湾漕耱囹铟眍 戾耱� � 桧忮眚囵�. 湾忸珈铈眍 铗牮� 洋礓箨 项觐痂蝈�� 祛疱� 漓� 70 箴钼��")
	...
	GiveItem ( role , 0 , 5886 , 1 , 103) 
	GiveItem ( role , 0 , 5887 , 1 , 103)  
	GiveItem ( role , 0 , 5888 , 1 , 103)  
	GiveItem ( role , 0 , 5889 , 1 , 103) 
	GiveItem ( role , 0 , 5890 , 1 , 103)  
	GiveItem ( role , 0 , 5891 , 1 , 103) 
	GiveItem ( role , 0 , 5892 , 1 , 103)
	GiveItem ( role , 0 , 5893 , 1 , 102)  
	GiveItem ( role , 0 , 5894 , 1 , 102) 
	GiveItem ( role , 0 , 5895 , 1 , 102)  
	GiveItem ( role , 0 , 5896 , 1 , 102) 
...
function my_ticket( role , Item )
	local a = CheckBagItem(role,4817)
	local b = ChaIsBoat(role)
	local hp = Hp(role) 
	local mxhp = Mxhp(role) 
	local sp = Sp(role) 
	local mxsp = Mxsp(role) 
		...
		--SystemNotice (role , "义脲镱痱圉�� 铟屙� 篁铎栩咫 镳铞羼�. 项驵塍轳蜞, 忸耨蜞眍忤蝈 锣 玟铕钼 � 爨眢")
		UseItemFailed ( role )
		return
	end
if b == 0 then
		if a > 0 then
			local d = DelBagItem(role,4817,1)
			if d == 1 then
				MoveCity(role,"Chaos Argent")
				return
			end
...
function Sk_Script_AdminX ( role , Item ) 											
	local sk_add = SK_HLKJ 
	local form_sklv = GetSkillLv( role , sk_add ) 
...
function Sk_Script_Gm_Mp ( role , Item ) 											
	local sk_add = SK_GM  
	local form_sklv = GetSkillLv( role , sk_add ) 
