...
QUIT_OUT=2	---答错后提示退出
function Quse01()
	AddQues("你是白痴吗",ASK_LINE,QUIT_OUT)
	AddNextQues()
	Ques("1+1=?")
	InitChoose()
	choose ("1")
...
function MxhpSb(a)
	--LuaPrint("Obtain character attribute mxhp_sb") 
...
function Hp_Dmg(role,dmg) --[[Damage Calculation in Chaos Argent]]--
	...
	local map_name_DEFER = GetChaMapName ( role )
	local Can_Pk_Darkblue2 = Is_NormalMonster (role)
	local T=0.35	
	if map_name_ATKER == "deathcircle" or map_name_DEFER == "deathcircle" then
		if Can_Pk_DeathCircle == 0 then
			dmg = dmg * T
		end
	end
...
--		local Killed_attr = ITEMATTR_VAL_DEX
	...
	if map_name_ATKER == "garner2" or map_name_DEFER == "garner2" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		local Notice_all = "["..def_name.."] was fucked up ["..atk_name.."]"
		Notice ( Notice_all )
	end
	...
	if map_name_ATKER == "deathcircle" or map_name_DEFER == "deathcircle" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		local Notice_all = "["..def_name.."] was Pwned by ["..atk_name.."]"
		Notice ( Notice_all )
	end
	
	if map_name_ATKER == "deathamtch" or map_name_DEFER == "deathmatch" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		local Notice_all = "["..def_name.."] got spank by ["..atk_name.."]"
		Notice ( Notice_all )
	end
	
	if map_name_ATKER == "yschurch" or map_name_DEFER == "yschurch" then
		local atk_name = GetChaDefaultName ( ATKER )
		local def_name = GetChaDefaultName ( DEFER )
		local Notice_all = "["..def_name.."] got rape in the anal by ["..atk_name.."]"
		Notice ( Notice_all )
	end
		   GiveItem ( ATKER , 0 , 7641  , 1 , 4 )
...
--	SystemNotice ( role_want_repair , "requires gold"..Money_Need )
	...
	if itemId == 1121 or itemId == 1122 then
		SystemNotice( role_want_repair , "Items can not be repaired" )
		return 0
	end
...
function cha_timer(role, freq, time) 
	...
	if math.mod(now_tick, resume_freq) == 0 and now_tick > 0 then
		MoJianBag(role,now_tick)-------Shadowbane on Inventory
		MoJianHand(role,now_tick)--------Shadowbane equipped
	end	
...
--		return
	...
	if math.mod(now_tick, pet_freq) == 0 and now_tick > 0 then 
		...
		if is_role_living == 1 then   
			...
			local ItemUse_siliao = GetChaItem ( role , 2 , 2  ) --取角色背包第三栏的指针
			local ItemUse_siliao_ID = GetItemID ( ItemUse_siliao ) --调用程序接口获得Item道具类型
----------------------------------------------------------------------08商品调整kokora------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------PET FRUIT FUNCTION END by zureXC--
-------------------------------------------------------------------------------------------------

--Pet Max Level begin by zureXC--
...
function AddExp_3 ( role )
	...
	elseif lv_role > 50 and lv_role <= 60 then
		exp_3 = math.floor(exp_up * 0.12)
	elseif lv_role > 60 and lv_role <= 70 then
		exp_3 = math.floor(exp_up * 0.06)
	else
		exp_3 = math.floor(exp_up * 0.036)
	end
...
function GiveZhongXuePrize(role,zblv,zbtype)
	...
	if count > 0 then
		...
		local name = GetChaDefaultName(role)
		LG("Search_err","GiveZhongXuePrize 没有找到",name,"的装备")
	end
end
...
--Class Changer Functions

function CH1_1 ( role,npc ) ------sowrdman
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)
	local cha_job = GetChaAttr(role, ATTR_JOB)
	
	if cha_type ==4 or cha_MY <= 10000 or cha_job ~= 0 then
		SystemNotice( role ,"You must have 10,000g And Be Newbi and be phillis/Crasis/Lance to be sowrdman")
		return
	end

	
	if cha_type ==1 or cha_type ==2 or cha_type ==3 and cha_job == 0 and cha_MY >= 10000 then
...
function CH1_2 ( role,npc )  ------hunter 
	...
	if cha_type ==2 or cha_type ==4 or cha_MY <= 10000 or cha_job ~= 0 then
		SystemNotice( role ,"You must have 10,000g And Be Newbi and be phillis/Lance to be Hunter")
		return
	end

	
	if cha_type ==1 or cha_type ==3 and cha_job == 0 and cha_MY >= 10000 then
		TakeMoney(role,0,10000)
	end
...
function CH1_3 ( role,npc )  ------Explorer 
	...
	if cha_type ==2 or cha_MY <= 10000 or cha_job ~= 0 then
		SystemNotice( role ,"You must have 10,000g And Be Newbi and be phillis/Lance/Ami to be Explorer")
		return
	end

	
	if cha_type ==1 or cha_type ==3 or cha_type ==4 and cha_job == 0 and cha_MY >= 10000 then
		TakeMoney(role,0,10000)
	end
...
function CH1_4 ( role,npc )  ------herblist
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)
	local cha_job = GetChaAttr(role, ATTR_JOB)
	
	if cha_type ==2 or cha_type ==1 or cha_MY <= 10000 or cha_job ~= 0 then
		SystemNotice( role ,"You must have 10,000g And Be Newbi and be phillis/Ami to be Herblist")
		return
	end

	
	if cha_type ==3 or cha_type ==4 and cha_job == 0 and cha_MY >= 10000 then
		TakeMoney(role,0,10000)
	end
...
function CH2_1 ( role,npc )  ------2nd class Cruz
	...
	if cha_type ==2 or cha_type ==4 or cha_MY <= 50000 or cha_job ~= 1 then
...
function CH2_2 ( role,npc )  ------2nd class Voy
	local cha_job = GetChaAttr(role, ATTR_JOB)
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)

	if cha_type ==2 or cha_MY <= 50000 or cha_job ~= 4 then
		SystemNotice( role ,"You must have 50,000g And Be a Explorer and be Phlliss/Lance/Ami to be Get To Be a Voyager")
		return
	end
...
function CH2_3 ( role,npc )  ------2nd class SS
	local cha_job = GetChaAttr(role, ATTR_JOB)
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)

	if cha_type ==2 or cha_type ==4 or cha_MY <= 50000 or cha_job ~= 2 then
		SystemNotice( role ,"You must have 50,000g And Be a Hunter and be Phlliss/Lance to be Get To Be a SharpShooter")
		return
	end
...
function CH2_4 ( role,npc )  ------2nd class Champ
	local cha_job = GetChaAttr(role, ATTR_JOB)
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)

	if cha_type ~=2 or cha_MY <= 50000 or cha_job ~= 1 then
		SystemNotice( role ,"You must have 50,000g And Be a Sowrdman and be Crasis to be Get To Be a Champion")
		return
	end
...
function CH2_5 ( role,npc )  ------2nd class SM
	local cha_job = GetChaAttr(role, ATTR_JOB)
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)

	if cha_MY <= 50000 or cha_job ~= 5 then
		SystemNotice( role ,"You must have 50,000g And Be a Herblist and be Phillis/Ami to be Get To Be a Seal Master")
		return
	end
...
function CH2_6 ( role,npc )  ------2nd class Cleric
	local cha_job = GetChaAttr(role, ATTR_JOB)
	local cha_type = GetChaTypeID ( role )	
	local cha_MY = GetChaAttr(role, ATTR_GD)

	if cha_MY <= 50000 or cha_job ~= 5 then
		SystemNotice( role ,"You must have 50,000g And Be a Herblist and be Phillis/Ami to be Get To Be a Cleric")
		return
	end
...
function MoJianBag(role,now_tick)
	...
	SystemNotice( role , "Shadowbane suck because of your slaughter, lost in the vast darkness of the China " )
		RemoveChaItem( role , 6347 , 1 , 0 , -1 , 2 , 1 )
	return
	end
	if math.mod( b , 180 ) == 0 and b < 1440 and b > 0 then 
		SystemNotice( role , "Shadowbane is only  "..math.floor(b/50).." durability" )
		local x, y = GetChaPos(role)
		local map_role = ChargeMapETC ( role )
		ScrollNotice(name.." is turning Invincible with Shadowbane at"..map_role.." region "..math.floor(x/100)..","..math.floor(y/100).." hovering near its powerful destructive to all around monster Fear! ", 1 )
		return
	end
end

function MoJianHand(role,now_tick)
	...
	if weapon_id == 6347 then
		...
		SystemNotice( role , "Shadowbane owner suck, Shadowbane abandoned its owner, Disappeared in the vast darkness. " )
			RemoveChaItem( role , 6347 , 1 , 0 , -1 , 2 , 1 )	
			return
		end
		if math.mod( a , 180 ) == 0 and a < 1440 and a > 0 then
			SystemNotice( role , "Shadowbane is only  "..math.floor(a/50).." durability" )
			local x, y = GetChaPos(role)
			local map_role = ChargeMapETC ( role )
			ScrollNotice(name.." is turning Invincible with Shadowbane at "..map_role.." region! "..math.floor(x/100)..","..math.floor(y/100).." hovering near its powerful destructive to all around monster Fear! ", 1 )
			return
...
function ChargeMapETC(role)
	...
	map_role_e[39] = "mingyun"
	local map_role_name={}
	map_role_name[1] = "Argent City"
	map_role_name[2] = "Shaitan City"
	map_role_name[3] = "Icicle Castle"
	map_role_name[4] = "Abandon Mine"
	map_role_name[5] = "Lone Tower"	
	map_role_name[6] = "Garden of Edel"
	map_role_name[7] = "Dark Swamp"
	map_role_name[8] = "Forsaken City"
	map_role_name[9] = "Forsaken City 2"
	map_role_name[10] = "Forsaken City 3"
	map_role_name[11] = "Demonic World"
	map_role_name[12] = "Demonic World 2"
	map_role_name[13] = "Party Challenge"
	map_role_name[14] = "Carribbean"	
	map_role_name[15] = "Argent City"
	map_role_name[16] = "Abaddon 1"
	map_role_name[17] = "Abaddon 2"
	map_role_name[18] = "Abaddon 3"
	map_role_name[19] = "Abaddon 4"
	map_role_name[20] = "Abaddon 5"
	map_role_name[21] = "Sacred War"
	map_role_name[22] = "Thundoria Mirage"
	map_role_name[23] = "Shaitan Mirage"
	map_role_name[24] = "Icicle Mirage"
	map_role_name[25] = "Mini Sacred War"
	map_role_name[26] = "Marriage Church" 
	map_role_name[27] = "Christmas Village"
	map_role_name[28] = "Boss Isle"
	map_role_name[29] = "Converted Christmas Village"	
	map_role_name[30] = "Prision"
	map_role_name[31] = "Arena A"
	map_role_name[32] = "Arena B"	
	map_role_name[33] = "Arena C"
	map_role_name[34] = "Winter Isle"
	map_role_name[35] = "Aurora 1"
	map_role_name[36] = "Aurora 2"
	map_role_name[37] = "Dark Area 1"
	map_role_name[38] = "Dark Area 2"
	map_role_name[39] = "Unknown Map"
	local i=0	
	for i=1,39,1 do
		if	map_role==map_role_e[i] then
			map_role=map_role_name[i]
...
function IsWaBao(role)
	...
	local ZBday = GetChaAttr(role,ATTR_EXTEND2)
	local ZBstate=GetChaAttr(role, ATTR_EXTEND1)
	local ZBstate1=math.floor(ZBstate/10)
	local ZBstate2=math.mod(ZBstate,10)
	local now_day=os.date("%d")
	now_day=tonumber(now_day)
	if now_day==ZBday and ZBstate2==3 and ZBNMain==0 then 
