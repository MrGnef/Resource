...
function GetChaName_born2 ( role )
	...
	local i = CheckBagItem( role, 5853 )    -------------------add the rebirth 2 rune Code here
	if i < 1 then
		SystemNotice( role ,"Please show your proof of rebirth 2")
		return 0
	end
	...
	local zs1 =TakeItem( role, 0,5853, 1 )  ------------------------------add the Rebirth 2 Rune Code here
	if zs1==0 then
...
function GetChaName1_born2 ( role,npc )
        ...
        local cha_type = GetChaTypeID ( role )
	if cha_type~=1 and cha_type~=3 then
		SystemNotice( role ,"Only Lance and Phillis can change into Crusader")
		return
	end
...
function LvUpSk_2 ( role )
	local cha_job = GetChaAttr(role, ATTR_JOB)
	local skc_add = SK_WYZ
	local skv_add = SK_CYN
	local skch_add = SK_BSJ
	local sks_add = SK_HLP
	local sksm_add = SK_EMZZ
	local skcl_add = SK_SSSP

	if cha_job == 8 then
		AddChaSkill ( role , skch_add , 2 , 1 , 0 )
	elseif cha_job == 9 then
		AddChaSkill ( role , skc_add , 2 , 1 , 0 )
	elseif cha_job == 12 then
		AddChaSkill ( role , sks_add , 2 , 1 , 0 )
	elseif cha_job == 13 then
		AddChaSkill ( role , skcl_add , 2 , 1 , 0 )
	elseif cha_job == 14 then
		AddChaSkill ( role , sksm_add , 2 , 1 , 0 )
	elseif cha_job == 16 then
		AddChaSkill ( role , skv_add , 2 , 1 , 0 )
	end
end
...
function GetChaName_born3 ( role )
   ...
   local cha_job = GetChaAttr(role, ATTR_JOB)
   -----------------???
   local i = CheckBagItem( role, 5854 )    
   if i < 1 then
   ...
   local zs1 =TakeItem( role, 0,5854, 1 )
   if zs1==0 then
   ...
   PlayEffect( npc, 361 )
   Notice("Extreme celebration "..cha_name.." 3rd Rebirth successful. Blessing from the whole server "..cha_name.." Hope you have a safe journey and everything goes your way!")
   return 1
