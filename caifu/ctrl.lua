...
function get_map_entry_pos_caifu()   --设置入口的位置的坐标（坐标（米））
...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2008/4/16/2/00", "0/8/0", "0/0/0", "0/2/0") --设置入口的时间，地图对象，首次开启时间（年/月/日/时/分），以后再次开启的间隔（日/时/分，全０表示只有首次开启），每次开启到入口消失的间隔（日/时/分，全０表示永不消失），每次开启到地图关闭的间隔（日/时/分，全０表示永不关闭）。。
