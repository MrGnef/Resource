...
function after_create_entry(entry) 
    ...
    Notice("Announcement: According to weather forecast, near Magical Ocean at ["..posx..","..posy.."] emerged a gigantic Whirlpool. All ships beware! There are reports that ships entered the Whirlpool and reached a gloomy place called [Dark Swamp].") --通知本组服务器的所有玩家
...
function after_player_login_darkswamp(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
    ChaNotice(player_name, "Announcement: According to weather forecast, near Magical Ocean at ["..posx..","..posy.."] emerged a gigantic Whirlpool. All ships beware! There are reports that ships entered the Whirlpool and reached a gloomy place called [Dark Swamp].") --通知本组服务器的所有玩家
