...
function get_map_entry_pos_darkblue2()   --设置入口的位置的坐标（坐标（米））
...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2006/10/18/14/0", "0/0/1", "0/0/50", "0/0/60") --设置入口的时间，地图对象，首次开启时间（年/月/日/时/分），以后再次开启的间隔（日/时/分，全０表示只有首次开启），每次开启到入口消失的间隔（日/时/分，全０表示永不消失），每次开启到地图关闭的间隔（日/时/分，全０表示永不关闭）。。
...
--function map_copy_first_run_darkblue2( map_copy )
...
function map_copy_run_special_darkblue2( map_copy ) 
	...
	Notice("乱斗冰狼堡中此时还存活着"..HowManyNoDead.."位玩家")
...
--	SystemNotice(role,How_Many_Active)
	...
	--	local message = "今天的乱斗共存活了"..How_Many_Active .. "位玩家,他们每人将获得"..Money_darkblue2 .."的金钱"
	--	SystemNotice(role,How_Many_Active)
		return
	end
	...
	if How_Many_Active <  6 and How_Many_Active > 1 then
		...
		--local message = "今天的乱斗中共存活了"..How_Many_Active .."位玩家,他们每人将获得"..Money_darkblue2 .."的金钱和1个乱斗之星徽章"
		--Notice ( message )
		return
	end
	...
	if How_Many_Active == 1 then
		...
		local message = "今天的乱斗中存活的只有"..cha_name.."他将获得 ".. Money_darkblue2 .."的金钱和1个乱斗之王徽章"
	--	SystemNotice(role,How_Many_Active)
