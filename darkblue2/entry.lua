...
function after_create_entry(entry) 
    ...
    Notice("海盗广播：深蓝群岛的["..posx..","..posy.."]位置发现一个通往《乱斗冰狼堡》的入口。") --通知本组服务器的所有玩家

end

function after_destroy_entry_darkblue2(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("海盗广播：通往《乱斗冰狼堡》的入口已经消失了，祝您愉快～！") 

end

function after_player_login_darkblue2(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
    ChaNotice(player_name, "海盗广播：深蓝群岛的["..posx..","..posy.."]位置发现一个通往《乱斗冰狼堡》的入口。") --通知本组服务器的所有玩家
...
function check_can_enter_darkblue2( role, copy_mgr )
	...
	if Team_In == 1 then
		SystemNotice ( role , "您现在处于组队状态,不能进入乱斗冰狼堡" )
		return 0
		
	end
	if FightingBook_Num <= 0 then
		SystemNotice ( role , "您身上没有勇者之证,不能进入乱斗冰狼堡,请到竞技管理员处领取勇者之证" )
		return 0
	...
	if HonorPoint < 20 then
		SystemNotice ( role , "您的荣誉点不够,不能进入乱斗冰狼堡" )
		return 0
	end

	if HonorPoint >30000 then
		SystemNotice ( role , "您的荣誉值过高,不能进入乱斗冰狼堡" )
		return 0
	end

	local Credit_darkblue2 = GetCredit(role)
	 if Credit_darkblue2 < 30 then 
		SystemNotice ( role , "您的名声值不够,不能进入乱斗冰狼堡" )
		return 0
	--else
--		DelCredit(role,30)
	end
	...
	if Lv(role) < 20 then
	SystemNotice(role, "进入乱斗冰狼堡角色等级必须在20级以上")
		return 0    
	end
	...
	if Has_money == 1 then
		...
		SystemNotice(role,"您所带的金钱不够,不能进入乱斗冰狼堡")
		return 0
	end
...
function begin_enter_darkblue2(role, copy_mgr) 
	...
	DelCredit(role,30)
		SystemNotice(role,"开始进入《乱斗冰狼堡》,收取金钱"..Money_Need..",扣除名声值30") 
		MoveCity(role, "乱斗冰狼堡")
