...
SetMapEntryTime(map, "2006/10/18/12/0", "0/4/0", "0/2/0", "0/2/0") --设置入口的时间，地图对象，首次开启时间（年/月/日/时/分），以后再次开启的间隔（日/时/分，全０表示只有首次开启），每次开启到入口消失的间隔（日/时/分，全０表示永不消失），每次开启到地图关闭的间隔（日/时/分，全０表示永不关闭）。。
...
if Now_Time == 24 or Now_Time == 4 or Now_Time == 8 or Now_Time == 12 or Now_Time == 16 or Now_Time == 20 then
return 1
else
return 1
end
