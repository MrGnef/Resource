...
function after_create_entry(entry) 
    ...
    Notice("Announcement: In Deep Blue region, players has discovered ["..posx..","..posy.."] emerges a portal that leads to [Icicle Mirage]. All players beware.")
...
function after_player_login_binglang2(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --取地图入口的位置信息（地图名，坐标，目标地图名）
    ChaNotice(player_name, "Announcement: In Deep Blue region, players has discovered ["..posx..","..posy.."] emerges a portal that leads to [Icicle Mirage]. All players beware.")
...
function check_can_enter_binglang2( role, copy_mgr )
	...
	if Lv(role) > 89 then
		SystemNotice(role, "Characters need to be below Lv 90 to enter Icicle Mirage")
		return 0    
	end
	...
	if Credit_Binglang2 < 10 then
		SystemNotice(role, "You do not have sufficient Reputation points. Unable to enter Icicle Mirage")
		return 0
	else
		DelCredit(role,10)
		return 1
