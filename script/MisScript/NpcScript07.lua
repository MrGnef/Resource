...
Talk( 1, " Привет, я меняю 3 стата на 6 келей! " ) 
	InitTrigger() 
	...
	Text( 1, " Я согласен заплатить такую цену за свою силу! ", MultiTrigger, GetMultiTrigger(), 1) 
	Talk( 2, " У тебя нет нужных предметов для обмена. " ) --Ошибка.
end
...
Talk( 1, " Привет. Я меняю гранд. тело на 300 келей!!! " ) --Слова НПС
	...
	TriggerFailure( 1, JumpPage, 2 )
	Text( 1, " То что нужно!  ", MultiTrigger, GetMultiTrigger(), 1)
	Talk( 2, " У тебя малость не хватает. " ) 
end

function wowah2 ()

Talk( 1, " Привет. я меняю 150 келей на гранд. руку!!! " ) --Слова НПС
	InitTrigger() --Тригер, обменика.
	...
	TriggerFailure( 1, JumpPage, 2 ) 
	Text( 1, " То что нужно! ", MultiTrigger, GetMultiTrigger(), 1) 
	Talk( 2, " У тебя малость не хватает. " ) --Ошибка.
end

function wowah3 ()

Talk( 1, " Привет я меняю гранд. ногу на 150 келей! " ) --Слова НПС
	InitTrigger()
	TriggerCondition( 1, HasItem, 3457, 150, 4 ) 
	TriggerCondition( 1, HasLeaveBagGrid, 1 )
	TriggerAction( 1, TakeItem, 3457, 150, 4 ) 
	...
	TriggerFailure( 1, JumpPage, 2 ) 
	Text( 1, " То что нужно! ", MultiTrigger, GetMultiTrigger(), 1) --Ваш ответ и запуск тригера, обмена.
...
Talk( 1, " Привет, я меняю бог кольцо на 50 кэлей! " ) 
InitTrigger() 
...
TriggerFailure( 1, JumpPage, 2 )
Text( 1, " C этими кольцами я буду бнепобедим! ", MultiTrigger, GetMultiTrigger(), 1)
Talk( 2, " Прости, но у тебя нету 50 кэлей! " )
end

function wowah6 ()

Talk( 1, " Привет, я меняю бог бижу на 70 кэлей! " ) --Слова НПС
	InitTrigger() 
	...
	TriggerFailure( 1, JumpPage, 2 )
	Text( 1, " С ней я буду не победим! ", MultiTrigger, GetMultiTrigger(), 1)
	Talk( 2, " Прости, но у тебя нет 70 келей. " )
end

function wowah7 ()

Talk( 1, " Привет! Я меняю Расписки новичка хаоса 5шт на один реф 4 лвл " ) --Слова НПС
	InitTrigger() --Тригер, обменика.
...
Talk( 1, " Привет! Я меняю Расписки эксперта хаоса 5шт на один реф 5 лвл " ) --Слова НПС
	InitTrigger() --Тригер, обменика.
	...
	Talk( 2, " Мне нужны Эксперт расписки хаоса! " ) --Ошибка.
end

function wowah9 ()

Talk( 1, " Привет!!! Я КОРОЛЬ Аргента, я уже очень стар и поэтому мне нужны наследники.Я отдам свою корону, тому кто докажет что он силён, мужествен, и смел! Чтобы получить мою корону, вам придется отправиться в мир демонов, и убить толпы монстров, с монстров подбирать Руны Джа, когда соберёш 50 штук, возвращайся ко мне, я отдам свою корону " ) --Слова НПС
	InitTrigger() --Тригер, обменика.
	...
	Talk( 2, " Ты меня решил обманул?! Казнить его!!! " ) --Ошибка.
end
