...
function JoJo_14 ()
	...
	Talk( 1, " Ангел 2 перерождения : Только лучшие солдаты и пираты имеют шанс переродится! ")
	Text( 1, " Больше о перерождении ", JumpPage, 2)
	Text( 1, " Я хочу переродиться ", JumpPage, 6)
	
	
	Talk( 2, " Ангел 2 перерождения :  Чтобы ты хотел узнать о перерождении? ")
	Text( 2, " Классы после перерождения ", JumpPage, 3)
	Text( 2, " Сброс статов и скилов после перерождения ", JumpPage, 4)
	Text( 2, " Условия для перерождения ", JumpPage, 5)
	
	Talk( 3, " Игроки могут изменить свой класс после перерождения : Ланс может стать Крузом,Воем или шарпом. Карциз может стать чемпионом. Филлис может стать Хиллером, Колдуньей, Воем или Шарпом. Ами может стать Хиллером , Колдуньей или Воем ")
	Text( 3, " Спасибо я все понял ", CloseTalk)

	Talk( 4, " Все атрибуты будут сброшены после перерождения и все статпоинты будет помещенны в карьерный лот . Все скиллпоинты и атрибуты будут доступны к перераспределению. ")
	Text( 4, " Спасибо теперь я все понимаю ", CloseTalk)

	Talk( 5, " Чтобы получить возможность пройти 2 перерождение, Ты должен пройти квест 2 перерождения может быть активирован у НПС рядом с ангелом. Найди меня когда получишь руну 2 перерождения или же ты можешь прийти ко мне, когда получишь карточку перерождения. ")
	Text( 5, " Спасибо я все понял ", CloseTalk)

	Talk( 6, " Ангел 2 перерождения: Получается ты выполнил этот квест? Выбери свой вариант. " )
	Text( 6, " Круз ",  GetChaName1_born2, 1)
	Text( 6, " Чамп ",  GetChaName2_born2, 1)
	Text( 6, " Хиллер ",  GetChaName5_born2, 1)
	Text( 6, " Вой ",  GetChaName3_born2, 1)
	Text( 6, " Шарп ", GetChaName4_born2, 1)   
	Text( 6, " Колдунья ", GetChaName6_born2, 1)
	Text( 6, " Я не хочу перерождаться во 2 раз ", CloseTalk)
